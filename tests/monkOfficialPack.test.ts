...
function action(id: string, sourceKey: string, key: string, label: string, economy: string, cost: number, effects: StoredMechanic extends never ? never : any[] = []): StoredMechanic {
  return {
    id,
    type: "action",
    sourceKey,
...
const levels: Array<{ level: number; mechanics: StoredMechanics }> = [
  {
    level: 1,
    mechanics: [
      feature("ma-rules", "monk-martial-arts", "martial_arts", "Боевые искусства", "Без доспеха и щита монах применяет правила Боевых искусств, включая куб урона и доступный бонусный безоружный удар после подходящего действия Атака."),
  ...
  },
  {
    level: 2,
    mechanics: [
      feature("focus-rules", "monk-focus", "monk_focus_rules", "Монашеская концентрация", "Запас Очков концентрации равен уровню монаха; весь потраченный запас восстанавливается после короткого или долгого отдыха. Способности концентрации используют этот конечный ресурс."),
      resource("focus-2", "monk-focus", "monk_focus", "Очки концентрации", 2, ["short_rest", "long_rest"], 2),
      feature("flurry-rules", "monk-flurry-of-blows", "flurry_rules", "Шквал ударов", "Бонусным действием монах тратит 1 Очко концентрации и делает два безоружных удара; с 10 уровня эта же способность делает три безоружных удара."),
      ...
      feature("uncanny-rules", "monk-uncanny-metabolism", "uncanny_metabolism_rules", "Невероятный метаболизм", "При броске инициативы монах может полностью восстановить Очки концентрации и восстановить HP по формуле способности. После использования способность возвращается только после долгого отдыха."),
      resource("uncanny-use", "monk-uncanny-metabolism", "monk_uncanny_metabolism", "Невероятный метаболизм", 1, ["long_rest"], 2),
      {
        ...
        effects: [
          { kind: "resource", key: "monk_focus", operation: "RESTORE", amount: 20 },
          { kind: "semantic", key: "heal", payload: { formula: "martial_arts_die + monk_level" } },
        ],
        tags: ["class", "initiative-trigger"],
      },
    ],
  },
  { level: 5, mechanics: [resource("focus-5", "monk-focus", "monk_focus", "Очки концентрации", 5, ["short_rest", "long_rest"], 5), value("ma-die-5", "monk-martial-arts", "martial_arts_die_sides", "Куб Боевых искусств", 8, 5), value("attacks-5", "monk-extra-attack", "attacks_per_attack_action", "Атак за действие Атака", 2, 5)] },
  { level: 10, mechanics: [resource("focus-10", "monk-focus", "monk_focus", "Очки концентрации", 10, ["short_rest", "long_rest"], 10), value("flurry-10", "monk-flurry-of-blows", "flurry_unarmed_strikes", "Ударов Шквала", 3, 10), value("speed-10", "monk-unarmored-movement", "unarmored_movement_bonus_feet", "Бонус Скорости без доспехов", 20, 10)] },
  { level: 11, mechanics: [value("ma-die-11", "monk-martial-arts", "martial_arts_die_sides", "Куб Боевых искусств", 10, 11)] },
  { level: 17, mechanics: [resource("focus-17", "monk-focus", "monk_focus", "Очки концентрации", 17, ["short_rest", "long_rest"], 17), value("ma-die-17", "monk-martial-arts", "martial_arts_die_sides", "Куб Боевых искусств", 12, 17)] },
  {
    level: 18,
    mechanics: [
      resource("focus-18", "monk-focus", "monk_focus", "Очки концентрации", 18, ["short_rest", "long_rest"], 18),
      feature("superior-rules", "monk-superior-defense", "superior_defense_rules", "Высшая защита", "В начале своего хода монах может потратить 3 Очка концентрации, чтобы на 1 минуту получить сопротивление всему урону, кроме урона силовым полем, пока не станет недееспособен."),
  ...
  { level: 20, mechanics: [resource("focus-20", "monk-focus", "monk_focus", "Очки концентрации", 20, ["short_rest", "long_rest"], 20), value("body-dex", "monk-body-and-mind", "body_and_mind_dexterity_increase", "Повышение Ловкости", 4, 20), value("body-wis", "monk-body-and-mind", "body_and_mind_wisdom_increase", "Повышение Мудрости", 4, 20), value("body-cap", "monk-body-and-mind", "body_and_mind_ability_cap", "Максимум Ловкости и Мудрости", 25, 20)] },
]
...
function bundleAt(level: number): CharacterTemplateBundle {
  return {
    ...
    template: {
      ...
      slug: "monk-core",
      name: "Монах",
      description: "Базовый класс монаха 2024.",
      version: 1,
      mechanics: [],
      choices: [],
      catalog_key: "class:monk",
      catalog_revision: "xphb-2024-monk-runtime-v1",
      source_kind: "official",
      source_label: "XPHB 2024",
      is_builtin: true,
      mechanical_summary: "Монах использует конечный запас Очков концентрации для боевых приёмов, ускоряется без доспехов и масштабирует куб Боевых искусств по уровню.",
      is_active: true,
      created_by: null,
...
test("Monk migration is base-class only and carries the strict integration markers", () => {
...
test("representative Monk package passes the strict class quality gate", () => {
...
test("Monk low, mid, and high levels resolve through parser into CE", () => {
  ...
  assert.equal(valueOf(high, "body_and_mind_dexterity_increase"), 4)
  assert.equal(valueOf(high, "body_and_mind_wisdom_increase"), 4)
  assert.equal(valueOf(high, "body_and_mind_ability_cap"), 25)
  assert.equal(high.actions.find((entry) => entry.key === "superior_defense")?.resourceCosts[0]?.amount, 3)
})

test("Uncanny Metabolism consumes its long-rest use and restores Focus to the actual max", () => {
  ...
  assert.ok(uncanny)
  assert.equal(uncanny.available, true)
