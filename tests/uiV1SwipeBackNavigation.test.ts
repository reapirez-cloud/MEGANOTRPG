...
test("ui v1 routes swipe, browser, and Telegram back through one navigation contract", async () => {
  ...
  assert.doesNotMatch(app, /screenOwnsTelegramBack/)
  assert.doesNotMatch(app, /if \(!canGoBack/)
  assert.match(characterView, /bindTelegramBackButton\(handleBack, \{ priority: 100 \}\)/)
