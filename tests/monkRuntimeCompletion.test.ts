...
function bundle(level: number): CharacterTemplateBundle {
  return {
    ...
    template: { id: "monk", campaign_id: "camp", kind: "class", slug: "monk-core", name: "Монах", description: "Монах 2024", version: 1, mechanics: [], choices: [], catalog_key: "class:monk", catalog_revision: "xphb-2024-monk-runtime-v1", source_kind: "official", source_label: "XPHB 2024", is_builtin: true, mechanical_summary: "Монах использует Очки концентрации, Боевые искусства и высокую мобильность с точной классовой прогрессией.", is_active: true, created_by: null, created_at: "2026-09-04T00:00:00Z", updated_at: "2026-09-04T00:00:00Z" },
    levels: [
...
function input(level: number, dexterity: number, wisdom: number): CharacterEngineInput {
  ...
  return {
    base: { id: "c", name: "Монах", level, abilities: { strength: 10, dexterity, constitution: 14, intelligence: 10, wisdom, charisma: 8 }, baseMaxHp: 100, baseSpeed: 30 },
...
test("completion migration installs native saves and Body and Mind", () => {
  for (const ability of ["strength","dexterity","constitution","intelligence","wisdom","charisma"]) {
    assert.match(completion, new RegExp(`savingThrow:'\\|\\|v_save|savingThrow:`))
