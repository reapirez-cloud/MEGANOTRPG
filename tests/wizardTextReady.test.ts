...
test("Wizard reference exposes the rebuilt base class and active subclass text catalog", () => {
  ...
  assert.match(wizard, /subclasses: \[\.\.\.wizardReferenceSubclasses, \.\.\.wizardTashaReferenceSubclasses, \.\.\.wizardSupplementReferenceSubclasses\]/)
})
...
test("Wizard ledger records the certified base class and all subclass runtime packages", () => {
  const wizard = wizardLedgerBlock()
  assert.match(wizard, /\*\*Text:\*\* `READY`/)
  assert.match(wizard, /\*\*Mechanics\/runtime:\*\* `READY`/)
  assert.match(wizard, /last_text_audit: 2026-09-01/)
  assert.match(wizard, /voss_base_class_feature_voice_pass: CONCRETE_GRIMDARK_BLACK_HUMOR_WITHOUT_PROFANITY_OR_DIRECT_INSULTS_2026_09_01/)
  assert.match(wizard, /last_mechanics_audit_started: 2026-08-31/)
  assert.match(wizard, /dev_base_class_runtime: READY/)
  assert.match(wizard, /subclasses: READY_ALL_13_RUNTIME_PACKAGES/)
  assert.match(wizard, /subclass_wave_0: READY_2026_08_31/)
  assert.match(wizard, /subclass_supported_count: 13/)
  assert.match(wizard, /physical spellbook[\s\S]*book-gated preparation/i)
  assert.match(wizard, /Spellbook as authoritative owned-spell state/)
  assert.match(wizard, /Arcane Recovery is implemented/i)
  assert.match(wizard, /long-rest cantrip replacement is a real once-per-long-rest server transaction/i)
  assert.match(wizard, /first chat message[\s\S]*closes both open Short Rest and Long Rest/i)
  assert.match(wizard, /defines and now installs exactly thirteen supported Wizard subclass identities/i)
  assert.match(wizard, /production_runtime: DEPLOYED_AND_CERTIFIED_2026_09_02/)
  assert.match(wizard, /production_subclass_revision: wizard-subclasses-runtime@3/)
})
