...
test("GM Voss receives actual campaign mutation tools and players do not", () => {
  ...
  assert.match(edge, /if \(authority === "player"\) return \[\][\s\S]*grantedCapabilities\.has\("campaign\.manage"\)[\s\S]*VOSS_MANAGER_TOOLS/)
...
test("admin Voss gets system security controls and full campaign read scope", () => {
  ...
  assert.match(edge, /grantedCapabilities\.has\("system\.admin"\) && authority === "admin"[\s\S]*VOSS_ADMIN_TOOLS/)
