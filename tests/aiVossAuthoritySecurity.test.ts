...
test("GM and owner never enter the Voss strike state machine", () => {
  const migration = read("supabase/migrations/20260919150000_voss_authority_security_v1.sql")
  const edge = read("supabase/functions/voss-agent/index.ts")

  assert.match(
    migration,
    /if v_is_owner or v_role <> 'player' or private\.is_system_admin\(p_user_id\) then[\s\S]*return query select 0, false, false, null::uuid/,
  )
...
test("player social-engineering strikes require high confidence and three strikes only block Voss", () => {
  ...
  assert.match(migration, /v_next_count >= 3/)
...
test("a third player strike writes an audit event and notifies system-admin Voss", () => {
  ...
  assert.match(migration, /'kind', 'ai_security_ban'/)
