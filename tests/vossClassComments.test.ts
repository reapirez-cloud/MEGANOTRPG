...
test("Reynar Voss has one explicit adventurer voice and worldview", () => {
  ...
  assert.match(vossVoice.magicStance, /не любит магию|опасн/i)
  assert.match(vossVoice.mundaneStance, /немагическ|профессионал/i)
  assert.match(vossVoice.clericStance, /чужими щитами|безопасного тыла/i)
  assert.match(vossVoice.clericStance, /Полевого лекаря.*под стрелами/i)
