...
test("Location management stays read-only until its dedicated interfaces are designed", () => {
  ...
  assert.match(locationNavigator, /Открыть локацию/)
  assert.match(locationNavigator, /Добавить вложенную локацию/)
...
test("Location previews open inline actions without native text selection", () => {
  ...
  assert.match(locationNavigator, /Добавить главную локацию/)
