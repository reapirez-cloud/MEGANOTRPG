...
- Stage 3 adds `ChatActionLauncher` as a pure first-level navigator. It accepts its action-section manifest as data, owns no CE/Snake execution and writes no state; selection is passed into the shared workspace through the drawer session `contentKey`. This leaves Stage 4 free to replace the temporary section manifest/content with actual capability/CE-backed actions without rebuilding launcher geometry.
