...
- Каскад записывает ещё не раскрытые функции в `pending_roles` с `coverage_complete=false`: увиденная с улицы таверна создаётся без вымышленных комнат. Младший знает ожидаемые роли архетипа, а DeepSeek V4.1 Flash получает reasoning effort `high` на основном и запасном маршрутах.
