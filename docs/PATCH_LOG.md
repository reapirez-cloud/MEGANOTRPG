...
- Added a dedicated `characterAbilitySnakeActions` domain provider. Stage 4 currently contributes only `Подробнее`; it is the single extension point reserved for Stage 5 manager suppression actions so collapsed/expanded renderers do not gain authority logic of their own.
