...
- Added `voss_authority_security_v1`: private AI-security event/state ledgers, atomic player strike counting, a hard database guard that refuses strikes for GM/owner/system-admin authority, three-strike Voss-only blocking, and service-role-only notification delivery into the system-admin Voss thread.
...
- Replaced scattered `owner/canManage` AI assumptions with one `player | gm | admin` authority ladder. GM manager tools now mutate Workshop characters, character life/publication state and locations through GM RLS; admin uses the same campaign-scoped tools through the server client and receives admin-only security inspection/unblock controls. Player security classification runs before the normal Voss answer and is defense-in-depth over the unchanged server/RLS permission boundary.
### Tests / verification
...
- Added Voss authority/security regressions locking the three-level authority ladder, player-only classifier path, severity/confidence strike threshold, database GM/owner/admin exemption, three-strike Voss-only block, real GM campaign manager tools and admin inspect/unblock controls.
