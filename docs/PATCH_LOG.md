...
- Added Bard Stage 5 subclass runtime for nine approved colleges: PHB 2024 Dance, Glamour, Lore and Valor plus official legacy/supplement Eloquence, Swords, Whispers, Creation and Spirits. Every subclass is parented to `class:bard`, unlocks from Bard level 3 and reuses shared CE/action/resource/spell/choice primitives.
- Added the missing College of Dance literary/reference package and exact 2024 rules, while keeping College of Tragedy explicit reference-only third-party material with no active builtin runtime template.
- Enforced the canonical Bardic Inspiration ledger across subclass spenders. Independent subclass pools such as Infectious Inspiration remain separate finite resources rather than being mistaken for copies of `bardic_inspiration`.
- Fixed the generic CharacterTemplateChoices rest path so `refresh=long_rest` choices use the authoritative rest-choice RPC. College of Spirits Spirit Session is the first Bard consumer of that corrected generic path.
- Stage 5 migration now self-certifies that every subclass action has an exact player-facing feature explanation under the same stable `sourceKey`, preventing SQL-authored packages from silently bypassing the TypeScript strict class-quality contract.
- Added Bard Stage 6 final certification as a fail-closed production package. `mechanics_status=READY` and the final revision are written only after the migration revalidates the entire Stage 1–5 stack, exact class progression, Inspiration/Expertise/Jack/spell contracts, all nine runtime colleges, resource identity and shared RPC availability.
- Promoted the player-facing Bard reference to runtime-backed through the shared class-reference layer. The nine certified colleges are runtime-backed; College of Tragedy remains explicit reference-only third-party material.
- Added `src/rule-templates/bardRuntimeCatalog.ts` as the canonical public/runtime roster so reference presentation and Stage 6 regression no longer duplicate the nine-college identity list.
...
- Added `tests/bardSubclassesStage5.test.ts` covering the nine-college runtime roster, parent Bard-level gating, canonical Bardic Inspiration costs, Tragedy exclusion, Lore/Spirits choice contracts, representative parser→CE resolution and the generic long-rest choice path.
- Stage 5 SQL dry-run initially caught and fixed a malformed Swords fighting-style helper call, then caught an over-broad BI audit that incorrectly classified Eloquence's independent Infectious Inspiration pool. The final migration dry-run passes completely inside `BEGIN … ROLLBACK`.
- Final pre-documentation Stage 5 CI is fully green: production build, lint and the complete repository suite pass (`981/981`).
- Applied `bard_subclasses_stage5_v1` to production (journal `20260911133259`). Live Bard revision is `xphb-2024-bard-stage5-subclasses-runtime-v1`, runtime stage 5, with exactly nine active child subclasses at unlock level 3 and zero active builtin Tragedy templates.
- Production spell-link audit reports Glamour 3, Lore 494 and Spirits 83 subclass spell links. BI-tagged actions with a noncanonical resource: `0`; subclass actions without matching exact feature explanations: `0`.
- Production transaction smoke verified parent-level gating (`Bard 2 → subclass locked`, `Bard 3 → source level 3`), the canonical Bardic Inspiration ledger and automatic creation of Glamour's `bard_glamour_beguiling_magic = 1/1` persistent resource. The full probe was rolled back.
- Stage 5 installer/upsert helpers remain closed to `anon` and `authenticated` and executable by `service_role`; Supabase security/performance advisors report no Bard/Stage5-specific findings.
- Added `tests/bardRuntimeFinalCertification.test.ts` covering strict fail-closed markers, parser→CE resolution, Bard parent-level semantics under a higher total character level, Class-tab CE presentation, shared GENA action/spell routes, generic Choice Runtime wiring, public runtime/reference boundaries and explicit generic debt markers.
- The first Stage 6 CI pass caught only certification-test drift: the synthetic player-facing feature text accidentally contained implementation-meta wording, and two older catalog regressions still expected Bard to be reference-only. Those stale expectations were corrected instead of weakening the strict class gate.
- Final pre-documentation Stage 6 CI is fully green: production build, lint and the complete repository suite pass (`990/990`).
- Dry-ran the complete final certification migration against production inside `BEGIN … ROLLBACK`; every fail-closed invariant passed before deployment.
- Pre-deploy and post-deploy transaction smokes both passed with full rollback. A total-level-12 character at Bard 3 resolved its subclass at Bard source level 3; a spent Bardic Inspiration use survived re-sync; a spent 3rd-level slot survived Bard 10→11; and the new 6th-level slot appeared at Bard 11.
- Applied `bard_runtime_final_certification_v1` to production (journal `20260911150941`). Live Bard is now `xphb-2024-bard-runtime-final-v1`, `runtime_stage=6`, `mechanics_status=READY`, `runtime_status=ready`, with all nine approved child subclasses also `ready` and Tragedy still inactive/reference-only.
- The final Bard campaign installer is `aaaaaaaal_campaigns_ensure_bard_runtime_final_v1`; private certifier/installer functions are not executable by `anon` or `authenticated` and remain available to `service_role` only.
- Post-deploy parity audit reports `0` broken Bard/subclass resource references, `0` duplicate mechanic IDs, nine ready runtime subclasses and zero active builtin Tragedy templates. Supabase security/performance advisors report no Bard/Stage6-specific findings; unrelated existing project-wide advisor debt was left untouched.
