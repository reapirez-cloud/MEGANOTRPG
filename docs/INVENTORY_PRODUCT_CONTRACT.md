...
> **Temporary active inventory UX override — 2026-09-22:** the spatial/Tetris presentation is isolated and preserved, but not mounted in the active character inventory. The active UX is a simple bag/slot projection over the same Cheburashka rows and holder tree: every canonical item instance or existing allowed stack occupies one slot; container capacity is derived from its authored `internal_grid_width × internal_grid_height`. This is a presentation/runtime placement override, not a second inventory truth and not deletion of the spatial engine.
