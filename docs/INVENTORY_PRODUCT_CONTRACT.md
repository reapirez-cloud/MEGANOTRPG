...
> **Superseding active inventory UX — 2026-09-26:** UI 1.0 uses a compact graphite inventory: five numbered quick-access shortcuts, compact equipment slots, one-row bag selector, and a searchable item list for the selected holder. Snake owns inspect (full art and description), use, equip, move, and quick-slot actions. The shortcuts reference existing Cheburashka rows and never change physical holder or equipped state. The spatial engine stays isolated. The previous all-bags/all-empty-cells layout is historical.

> **Historical active inventory UX override — 2026-09-22:** the spatial/Tetris presentation is isolated and preserved, but not mounted in the active character inventory. The active UX is a simple bag/slot projection over the same Cheburashka rows and holder tree: every canonical item instance or existing allowed stack occupies one slot; every bag is shown as its own always-visible panel with empty capacity cells; equipment renders all supported slots even when empty; drag/drop moves or swaps the canonical item rows on desktop and touch. A single `quick_access` marker is only a shortcut reference to an existing item and never changes its holder, hand, equipment or ownership state. Container capacity remains derived from authored `internal_grid_width × internal_grid_height`. This is a presentation/runtime placement override, not a second inventory truth and not deletion of the spatial engine.
