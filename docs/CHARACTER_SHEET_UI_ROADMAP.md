...
- certification also includes a real resolver-level multiclass isolation case: two classes may have different class levels, and each subclass inherits only its own parent class level instead of total level, its stored subclass level or the other class level;
- current Stage 15 coverage is 32 tests across the behavior + certification suites; GitHub Actions Node 24 confirmed all Stage 15 cases passing, including the resolver-level multiclass case;
- the same CI run confirmed TypeScript build and lint passing after exporting `ResolvedSourceRef` through the public Character Engine index;
- the repository-wide test step still has 7 pre-redesign failures unrelated to Stage 15, all expecting retired Character View / Inventory / Snake UI structures; they are tracked as obsolete test debt rather than used as a reason to resurrect removed UI;
