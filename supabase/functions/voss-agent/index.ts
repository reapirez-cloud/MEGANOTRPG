  ...
  const systemPrompt = [
    ...
    ...(canManage
      ? [
          ...
          "Не создавай заранее NPC, предмет или локацию только потому, что они нужны будущему этапу. Создай quest target placeholder, например «Где-то в лесу». Когда сущность действительно впервые входит в канон и target ещё placeholder, используй materialize_quest_target: он атомарно создаёт сущность, привязывает target и запускает Resolver. Если нужная каноническая сущность уже существует, ничего не дублируй и используй bind_quest_target.",
          ...
          "Повторный materialize_quest_target для уже bound target безопасен и должен вернуть существующую привязку, а не создавать дубль. Не разрывай materialization на отдельные create_* + bind вызовы без необходимости: иначе между ними возникает неканоническое промежуточное состояние.",
          ...
          "Location secret — это постоянная скрытая часть канонического мира, привязанная к конкретной локации: тайна, скрытая опасность, ресурс, персона, объект, путь или сюжетный крючок, который должен пережить отдельный квест и сессию. Для этого используй upsert_location_secret.",
          "Не складывай в location secrets будущие этапы и внутренние условия конкретного квеста: это остаётся в Quest Engine secrets. И наоборот, постоянную правду о локации не держи только в заметке квеста.",
          "read_location для GM/Admin возвращает locationSecrets и последние ревизии. Игрок не должен узнавать даже о существовании скрытого секрета из ответа помощника.",
          "reveal_text внутри location secret — только подготовленная формулировка для возможного раскрытия. Изменение status на resolved не публикует этот текст игроку автоматически. Публичное знание меняй отдельно через канонические world/lore данные.",
          "Не удаляй секреты ради уборки истории. active означает актуальную скрытую правду, resolved — ситуация разрешена, retired — больше не актуально. set_location_secret_state сохраняет историю ревизий.",
