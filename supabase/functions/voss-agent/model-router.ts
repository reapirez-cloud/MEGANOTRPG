...
export function classifyVossTask(
  ...
  if (
    /\b(запомни|запомнить|зафиксируй|зафиксировать|добавь\s+в\s+память)\b/u.test(text) ||
    /\bсохрани(?:ть)?\b.{0,40}\b(сводк|итог|резюме|факт|памят)/u.test(text)
  ) {
    return "memory_write"
  }
  ...
  if (
    draftContext &&
    /\b(исправ|измени|изменить|передел|убери|удали|замени|добавь|дополни|сократи|переимен)/u.test(text)
  ) {
    return "draft_edit"
  }

  const creationVerb =
    /\b(создай|создать|сделай|сделать|придумай|спроектируй|собери|сгенерируй|подготовь)\b/u.test(text)
  const contentNoun =
    /\b(зон|локац|нпс|npc|персонаж|предмет|оруж|брон|заклин|эффект|состояни|болезн|монстр|противник|механик|награ|достижен|трактир|подземел|квест)/u.test(text)
  ...
  if (
    /\b(напомни|вспомни|вспомин|истори[яю]|хронолог)/u.test(text) ||
    /\bчто\s+(?:было|произошло|случилось)\b/u.test(text) ||
    /\bкогда\s+(?:мы|они|он|она|группа)\b/u.test(text) ||
    /\bпрошл(?:ой|ую|ая|ые|ых)?\s+(?:сесси|сцен)/u.test(text) ||
    /\bкто\s+обещал\b/u.test(text) ||
    /\bпочему\s+(?:мы|группа)\b/u.test(text) ||
    /\bдо\s+этого\b/u.test(text)
  ) {
    return "memory_read"
  }
  ...
  if (
    hasEntity ||
    /\b(класс|подкласс|способност|умени|механик|заклин|предмет|инвентар|персонаж|локац|зон|эффект|состояни|правил)/u.test(text)
  ) {
