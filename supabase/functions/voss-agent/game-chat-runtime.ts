...
import { cascadeChildLimit } from "./location-hierarchy.ts"
...
const STAGE27_INVENTORY_EXECUTOR_TOOL = {
  ...
  function: {
    ...
    description:
      "Stage 27 post-turn inventory mutation. Use action=batch with deltas[] for linked effects. Grants are placed by Cheburashka into a free bag cell, then a free hand. A full inventory returns accepted=false, outcome=no_space as a normal resolved result: no item was taken and no linked delta was applied. Do not repeat until space is freed. Currency of one denomination uses one stack with physical weight. Equipment needs a compatible equipment_slot (e.g. dagger=main_hand, clothes=chest). The Item Registry resolves the canonical definition before Cheburashka creates the item.",
    parameters: {
      type: "object",
      ...
      properties: {
        ...
        item: {
          ...
          properties: {
            ...
            equipment_slot: {
              type: "string",
              enum: ["main_hand", "off_hand", "two_hands", "head", "neck", "shoulders", "chest", "hands", "wrists", "waist", "legs", "feet", "back", "ring_left", "ring_right", "ammo", "other"],
              description: "Required for new equipment without a known slot in its canonical definition. Use the actual wearable/wieldable slot.",
            },
...
const STAGE18_POST_TURN_WORKER_SYSTEM = [
  ...
  "В children передавай только нужные непосредственные опорные места: город 2–4 многофункциональных района, дорога 0–1 уже встреченный участок, другие зоны 0–3. Если внутренних узлов в опубликованной сцене нет, children=[]. Не создавай следующую улицу, комнату или поворот заранее. Никогда не строй grandchildren.",
  "Для таверны ожидаемые роли public_hall/service/storage, для постоялого двора ещё guest_area. Если герой только увидел вывеску с улицы, передай children=[]: сервер сохранит нераскрытые роли как pending_roles. Не выдумывай зал, склад или кухню. omitted_roles означает доказанное отсутствие функции, не неизвестность.",
  "source_location_id — откуда персонаж пришёл, parent_location_id — внутри какого места находится цель. Прибытие с дороги в город связывай переходом, не делай город дочерней частью дороги.",
...
const WORLD_MATERIALIZER_SYSTEM = [
  ...
  "Каскад строится максимум на один структурный уровень вниз только у реально посещённого узла. Держи карту компактной: город → 2–4 опорных района с несколькими функциями у каждого; дорога → 0–1 реально встреченный разворот/участок; район, таверна, лес или подземелье → 0–3 нужных опорных места. Мелкие улицы, комнаты, массовка и будущие точки остаются описанием до фактического посещения/необходимости.",
  ...
  "Функциональное покрытие важнее числа карточек. Для города сервер требует роли residential/commerce/governance/security/transit/services; один район может покрывать несколько ролей. Не создавай шесть районов только ради шести тегов и не помечай реально существующую функцию как omitted ради экономии. Если функция действительно отсутствует, укажи конкретную причину.",
  "Пустой children=[] — нормальный результат для дороги, пустоши или небольшого места без уже установленных внутренних узлов. Не создавай локацию для каждого поворота, шага, реплики, арта или описательной детали. Не раскрывай следующий уровень лишь потому, что родитель был посещён.",
  "Для таверны ожидаются public_hall/service/storage, для постоялого двора также guest_area. Если интерьер ещё не раскрыт, оставь children=[]; сервер запишет эти функции как pending_roles и заполнит их позже по мере раскрытия. Не выдумывай комнаты ради полного покрытия и не помечай неизвестность как omitted_roles.",
  ...
  "Создавай сюжетные сущности только по ТЗ. Каркас текущей локации должен помогать ориентироваться, а не механически заполнять карту; когда канон не даёт осмысленных дочерних мест, оставь children пустым.",
  "Не создавай глубже одного структурного слоя, массовых NPC или предметы ради заполнения. Проверяй уже существующие соседние и дочерние узлы и используй их вместо дублей.",
...
const STAGE12_GAME_MASTER_SYSTEM = [
  ...
  "Инвентарь физический: смотри source_inventory_room и canonical_inventory_for_present_characters. Для нового предмета сервер сам ищет свободную ячейку сумки, затем руку; root/«при себе» не хранилище. Если места нет и не пополняется существующая стопка, опиши, что персонаж не смог взять вещь и её надо оставить или освободить место; не утверждай, что предмет получен, и не создавай inventory grant intent. recent_no_space означает, что прошлая выдача была штатно отклонена: вещь не оказалась в инвентаре, не повторяй её автоматически. При сомнении формулируй получение осторожно.",
  ...
  "Stage 26: если финальный ответ устанавливает, что source_character физически вошёл/прибыл/остался в новой постоянной локации, обязательно добавь ОДИН location post_turn_intent, в котором явно указаны destination, parent/источник если известны и требование переместить source_character. Junior выполнит это одним materialize_location_cascade; внутренние узлы добавляются только когда они нужны для уже видимой сцены или компактного каркаса города.",
  ...
  "Для request_player_roll НЕ указывай request_type/ability_key/skill_key/attack_kind/modifier и не думай о RPC/API. Укажи roll_request: character_id, adjudication_mode(check|impossible_exact), uncertainty_scope(character_performance|world_discovery), canonical_evidence, resolver_decision_key, exact_goal, semantic_check обычным языком D&D, logical_difficulty(very_easy|easy|moderate|hard|very_hard|nearly_impossible), dc_visibility=public, success_envelope, failure_envelope, partial_success_envelope, label, reason. Игрок видит СЛ и свои шансы до броска. Для check success/failure envelopes обязательны; для impossible_exact обязательны failure + partial_success, а exact goal остаётся false.",
...
async function requestJuniorCompletionWithFallback({
  ...
  const effectiveReasoningEffort: JuniorReasoningEffort =
    ["gpt-5.6-luna", "deepseek-v4.1-flash"].includes(model.model_key)
  ...
  try {
    ...
    const fallbackReasoningEffort: JuniorReasoningEffort =
      ["gpt-5.6-luna", "deepseek-v4.1-flash"].includes(fallbackModel.model_key)
        ? "high" : "low"
...
function worldMaterializerValidationError(name: string, args: JsonRecord) {
  if (name === "materialize_location_cascade") {
    ...
    if (args.children.length > cascadeChildLimit(args.archetype)) {
      return "world_materializer_cascade_children_exceed_compact_limit"
    }
  ...
  if (name === "batch_location_changes") {
    ...
    if (operations.filter((raw) => jsonRecord(raw).op === "create").length > 4) {
      return "world_materializer_batch_location_create_limit"
    }
...
async function runStage18Intent({
  ...
  if (toolName === "materialize_location_cascade") {
    if (
      context.sourceLocation?.id &&
      toolArgs.move_character_id === sourceCharacterId &&
      toolArgs.location_id !== context.sourceLocation.id &&
      !toolArgs.source_location_id
    ) {
      toolArgs.source_location_id = context.sourceLocation.id
    }
    const prepared = await prepareLocationCascadeInput(admin, campaignId, toolArgs)
    if (prepared.error || !prepared.input) {
      throw new Error(prepared.error || "location_cascade_input_invalid")
    }
    toolArgs = prepared.input
  }
