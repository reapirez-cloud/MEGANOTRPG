...
const STAGE27_INVENTORY_EXECUTOR_TOOL = {
  ...
  function: {
    ...
    description:
      "Stage 27 post-turn inventory mutation. Use action=batch with deltas[] for linked effects. A grant needs a free bag cell or hand; full inventory rejects the entire transaction with inventory_no_free_slot. Do not repeat a capacity rejection; the failure receipt records that the item was not added until a real slot is freed. Currency of one denomination uses one stack with physical weight. Equipment needs a compatible equipment_slot (e.g. dagger=main_hand, clothes=chest). The Item Registry resolves the canonical definition before Cheburashka creates the item.",
    parameters: {
      type: "object",
...
const STAGE18_POST_TURN_WORKER_SYSTEM = [
  ...
  "Для выдачи предмета нужно свободное место в сумке или руке. Корень «при себе» не является хранилищем. Если сервер ответил inventory_no_free_slot, предмет не добавился: не повторяй ту же команду, пока игрок не освободит реальную ячейку. Экипировке указывай правильный equipment_slot: кинжал main_hand, одежда chest. Монеты одного номинала занимают одну ячейку стопкой, но сохраняют вес.",
...
const STAGE12_GAME_MASTER_SYSTEM = [
  ...
  "Инвентарь физический: переносимое новое имущество требует свободной ячейки сумки или одной из двух рук. Не представляй старое root/«при себе» как место с бесконечной ёмкостью. Если место неизвестно, формулируй получение предмета осторожно; при отказе inventory_no_free_slot предмет не оказался у персонажа, сначала нужно освободить ячейку.",
