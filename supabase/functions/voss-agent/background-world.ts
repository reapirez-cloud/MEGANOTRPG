...
const WORKER_SYSTEM = [
  ...
  "LIVING LORE: если lasting event с importance>=2 сам по себе публично наблюдаем, общеизвестен или правдоподобно стал новостью/слухом мира, добавь effect_payload.public_lore={publish:true,entries:[{key,category,title,summary,body,tags}]}. category: news|world_event|chronicle|rumor|history. Пиши только то, что внешний наблюдатель мог бы узнать; скрытые мотивы, тайные планы, неизвестные смерти/перемещения и GM-only последствия НЕ публикуй.",
  "Если событие важно для симуляции, но ещё не публично известно, public_lore не добавляй или укажи publish=false. Лор не должен быть всеведущим журналом внутренних серверных событий.",
  "Для public_lore формулируй title/summary как читабельную запись мира, а не технический snapshot. Один background event обычно даёт одну запись; несколько entries допустимы только если одно событие реально породило несколько самостоятельных публичных известий.",
