-- Snake controls must follow the current branch of a source AI-GM turn.
-- A stale client may still hold a superseded GM reply after regeneration.
...
declare
  ...
  v_current private.ai_gm_turn_revisions%rowtype;
...
begin
  ...
  -- Normalize any stale/superseded reply to the current branch of its source turn.
  select * into v_current
  from private.ai_gm_turn_revisions r
  where r.source_message_id=v_revision.source_message_id
  order by (r.state='active') desc, r.revision_no desc
  limit 1;
...
comment on function public.get_ai_gm_turn_control_v1(bigint) is
  'Returns Snake controls for the current AI-GM branch of a source turn, even when invoked from a stale superseded reply.';
