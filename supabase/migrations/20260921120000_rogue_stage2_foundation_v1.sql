...
declare
  ...
  v_features jsonb:='[
    {"level":1,"key":"sneak-attack","name":"Скрытая атака","description":"Один раз за ход при подходящем попадании оружием со свойством Finesse или Ranged можно добавить урон Скрытой атаки. CE хранит точную прогрессию кубов; сценические условия подтверждает мастер."},
    ...
    {"level":2,"key":"cunning-action","name":"Хитрое действие","description":"Бонусным действием можно совершать Рывок, Отход или Скрыться. Исполняемый action-runtime подключается на Stage 3."},
    ...
    {"level":3,"key":"rogue-subclass","name":"Подкласс разбойника","description":"На 3 уровне выбирается подкласс Разбойника. Stage 2 сохраняет точку разблокировки, но не устанавливает механики подклассов."},
    ...
    {"level":5,"key":"cunning-strike","name":"Хитрый удар","description":"При Скрытой атаке можно пожертвовать кубами её урона ради специальных эффектов. Исполняемые riders подключаются на Stage 3."},
    ...
    {"level":7,"key":"reliable-talent","name":"Надёжный талант","description":"Для подходящих проверок навыков и инструментов низкий результат к20 заменяется на 10. Полный runtime подключается на Stage 4."},
    ...
    {"level":20,"key":"stroke-of-luck","name":"Мастерский удар","description":"После проваленного D20 Test можно превратить результат к20 в 20; одно использование восстанавливается после короткого или долгого отдыха. Полный roll-runtime подключается на Stage 4."}
  ]'::jsonb;
begin
