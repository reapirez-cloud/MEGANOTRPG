-- Stage 17 reconciled foundation snapshot.
-- This repository was behind the deployed database when Stage 17 was built.
-- The original remote migration version is preserved, while this file intentionally
-- describes the final Stage 17 foundation needed to reproduce the deployed schema
-- from a clean Stage 16 database.
...
create table if not exists public.ai_player_intent_adjudications (
  ...
  source_intent_fingerprint text not null check (source_intent_fingerprint ~ '^[0-9a-f]{64}$'),
  ...
  adjudication_mode text not null check (
    adjudication_mode in ('deterministic_success','deterministic_failure','check','impossible_exact')
  ),
  exact_goal text not null check (length(btrim(exact_goal)) between 1 and 1600),
  exact_goal_allowed boolean not null,
  semantic_mechanic_request text not null default '' check (length(semantic_mechanic_request) <= 1600),
  logical_difficulty text check (
    logical_difficulty is null or logical_difficulty in (
      'very_easy','easy','moderate','hard','very_hard','nearly_impossible'
    )
  ),
  normalized_request_type text check (
    normalized_request_type is null or normalized_request_type in ('skill','ability','save','attack','custom')
  ),
  normalized_ability_key text,
  normalized_skill_key text,
  normalized_attack_kind text,
  dc integer check (dc is null or dc between 1 and 40),
  dc_visibility text not null default 'hidden' check (dc_visibility in ('public','hidden')),
  natural_20_policy text not null check (natural_20_policy in ('not_applicable','total_only','partial_only')),
  success_envelope text not null default '' check (length(success_envelope) <= 2400),
  failure_envelope text not null default '' check (length(failure_envelope) <= 2400),
  partial_success_envelope text not null default '' check (length(partial_success_envelope) <= 2400),
  receipt_fingerprint text not null unique check (receipt_fingerprint ~ '^[0-9a-f]{64}$'),
  created_at timestamptz not null default now(),
  uncertainty_scope text not null default 'character_performance'
    check (uncertainty_scope in ('character_performance','world_discovery')),
  canonical_evidence jsonb not null default '[]'::jsonb
    check (jsonb_typeof(canonical_evidence)='array'),
  world_proof_kind text not null default 'not_required'
    check (world_proof_kind in (
      'not_required','canonical_entities','resolver_exists','resolver_absent','impossible_exact'
    )),
  resolver_receipt_id uuid references public.ai_world_random_receipts(id) on delete restrict,
  resolver_decision_key text,
  unique (gm_job_id, sequence_no)
);

alter table public.pending_player_roll_requests
  add column if not exists adjudication_id uuid
  references public.ai_player_intent_adjudications(id) on delete restrict;
...
for each row execute function private.reject_ai_player_intent_adjudication_update_v1();
...
for each row execute function private.enforce_stage17_player_roll_freeze_v1();
