-- AI world: allow partial structural knowledge without inventing interiors.
-- Missing functions remain pending in the manifest; a later visited scene can fill them.
...
begin
  ...
  -- Unobserved interiors are unknown, not absent. Preserve every uncovered
  -- role as pending instead of fabricating rooms or failing the whole turn.
  select coalesce(array_agg(role order by role), '{}'::text[])
    into v_pending_roles
  from unnest(v_required_roles) as t(role)
  where not (role = any(v_covered_roles))
    and not (
      v_omitted ? role
      and btrim(coalesce(v_omitted->>role,'')) <> ''
    );

  v_coverage := v_coverage || jsonb_build_object(
    'coverage_complete',cardinality(v_pending_roles)=0,
    'cascade_depth',1,
    'required_roles',to_jsonb(v_required_roles),
    'covered_roles',to_jsonb(v_covered_roles),
    'pending_roles',to_jsonb(v_pending_roles),
