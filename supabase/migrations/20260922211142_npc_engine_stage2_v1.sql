create table public.npc_profiles (
  ...
  challenge_rating numeric(7,3) not null default 0,
  occupation text not null default '',
  faction text not null default '',
  appearance text not null default '',
  demeanor text not null default '',
  ...
  constraint npc_profiles_role_not_blank check (length(btrim(role)) > 0),
  constraint npc_profiles_creature_type_not_blank check (length(btrim(creature_type)) > 0),
  constraint npc_profiles_size_check check (
    size in ('tiny','small','medium','large','huge','gargantuan')
  ),
  constraint npc_profiles_cr_check check (
    challenge_rating >= 0 and challenge_rating <= 100
  )
);

comment on table public.npc_profiles is
  'NPC-only world identity metadata. D&D ability scores, combat values and character biography remain canonical in character_sheets/characters.';
comment on column public.npc_profiles.role is
  'Narrative/game role such as merchant, guard, quest_giver, noble, criminal or monster.';
comment on column public.npc_profiles.gm_notes is
  'GM/AI-only NPC context. Do not expose to players.';
comment on column public.npc_profiles.challenge_rating is
  'Optional D&D-style challenge rating. Fractional CR values are stored as decimals, e.g. 0.125 for 1/8.';

create index npc_profiles_campaign_idx
  on public.npc_profiles(campaign_id);
create index npc_profiles_role_idx
  on public.npc_profiles(campaign_id, role);
create index npc_profiles_tags_idx
  on public.npc_profiles using gin(tags);

CREATE OR REPLACE FUNCTION private.npc_profile_touch_updated_at_v1()
 RETURNS trigger
 LANGUAGE plpgsql
 SET search_path TO ''
AS $function$
...
revoke all on function private.npc_profile_touch_updated_at_v1() from public, anon, authenticated;

create trigger npc_profiles_touch_updated_at
before update on public.npc_profiles
for each row execute function private.npc_profile_touch_updated_at_v1();

CREATE OR REPLACE FUNCTION private.validate_npc_profile_v1()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
begin
  ...
  if v_character_type <> 'npc' then
    raise exception 'NPC profile can only belong to an NPC character';
  end if;

  if new.campaign_id <> v_campaign_id then
    raise exception 'NPC profile must belong to the character campaign';
  end if;
...
revoke all on function private.validate_npc_profile_v1() from public, anon, authenticated;

create trigger npc_profiles_validate
before insert or update on public.npc_profiles
for each row execute function private.validate_npc_profile_v1();

alter table public.npc_profiles enable row level security;
revoke all on table public.npc_profiles from anon;
grant select, insert, update, delete on table public.npc_profiles to authenticated;

create policy npc_profiles_manager_read
on public.npc_profiles
for select
to authenticated
using ((select private.can_manage_character(character_id)));

create policy npc_profiles_manager_insert
on public.npc_profiles
for insert
to authenticated
with check ((select private.can_manage_character(character_id)));

create policy npc_profiles_manager_update
on public.npc_profiles
for update
to authenticated
using ((select private.can_manage_character(character_id)))
...
create policy npc_profiles_manager_delete
on public.npc_profiles
for delete
to authenticated
using ((select private.can_manage_character(character_id)));
...
revoke all on function public.create_world_npc_v1(uuid, jsonb) from public, anon;
grant execute on function public.create_world_npc_v1(uuid, jsonb) to authenticated;
...
revoke all on function public.update_world_npc_v1(uuid, jsonb) from public, anon;
grant execute on function public.update_world_npc_v1(uuid, jsonb) to authenticated;

comment on function public.create_world_npc_v1(uuid, jsonb) is
  'Atomically creates a published canonical world NPC with D&D sheet data, NPC profile, position/habitats, optional relationship, and optional discovery state.';
comment on function public.update_world_npc_v1(uuid, jsonb) is
  'Atomically patches a canonical world NPC across character, D&D sheet, NPC profile, position/habitats, relationship and discovery state.';
