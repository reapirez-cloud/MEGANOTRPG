-- Keep the human-readable resource label when consuming an already-persisted
-- character resource. Previously v_label was only initialized for a newly
-- inserted state row, so an exhausted Focus/other persistent resource raised
-- `Недостаточно ресурса: <NULL>`.
  ...
  for v_cost in select value from jsonb_array_elements(p_costs) loop
    ...
    if v_state_key ~ '^spell_slot_[1-9]$' then
      ...
      if v_slots is null then
        raise exception 'Spell slots are unavailable';
      end if;

      v_max := coalesce((v_slots -> v_level::text ->> 'max')::integer, 0);
      v_used := coalesce((v_slots -> v_level::text ->> 'used')::integer, 0);
      if v_max - v_used < v_amount then
        raise exception 'Недостаточно ячеек % уровня', v_level;
      end if;
    ...
    select current, max_snapshot, label
