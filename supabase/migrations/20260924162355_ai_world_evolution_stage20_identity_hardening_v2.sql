...
create index if not exists npc_identity_versions_campaign_idx
  on public.npc_identity_fingerprint_versions(campaign_id,character_id,version desc);

create index if not exists npc_identity_evolution_campaign_idx
  on public.npc_identity_evolution_receipts(campaign_id,character_id,new_version desc);

create index if not exists npc_identity_fingerprints_last_major_event_idx
  on public.npc_identity_fingerprints(last_major_event_id)
  where last_major_event_id is not null;

create index if not exists npc_identity_observations_campaign_idx
  on public.npc_identity_observations(campaign_id,npc_character_id,observer_character_id);

create index if not exists npc_identity_observations_source_event_idx
  on public.npc_identity_observations(source_event_id);
...
begin
  ...
  v_campaign_id:=nullif(v_base->>'campaign_id','')::uuid;
  ...
  v_active_character_id:=nullif(v_base->>'active_character_id','')::uuid;
...
comment on function public.read_world_npc_dossier_v2(uuid) is
  'Stage 20 NPC dossier, SECURITY INVOKER. Base visibility comes from dossier v1; Stage 20 identity/observations additionally pass their own RLS. Anonymous sign-in users are rejected.';
