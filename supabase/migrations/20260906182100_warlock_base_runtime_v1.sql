...
create or replace function private.warlock_action(
  ...
  select jsonb_build_object(
    ...
    'range',jsonb_build_object('kind','self'),'resourceCosts',coalesce(p_costs,'[]'::jsonb),
    'effects',coalesce(p_effects,'[]'::jsonb),'tags',jsonb_build_array('warlock'),
...
begin
  ...
  into v_old_m,v_old_c from public.rule_template_levels
  ...
  from jsonb_array_elements(v_old_m) e(value)
  where coalesce(e.value->>'sourceKey','') not like 'warlock-base:%';

  select coalesce(jsonb_agg(e.value),'[]'::jsonb) into v_keep_c
  from jsonb_array_elements(v_old_c) e(value)
  where coalesce(e.value->>'key','') not like 'warlock_mystic_arcanum_%';

  update public.rule_template_levels
  set mechanics=v_keep_m||coalesce(p_mechanics,'[]'::jsonb),
      choices=v_keep_c||coalesce(p_choices,'[]'::jsonb)
  where template_id=p_template_id and level=p_level;
end;
$$;
...
begin
  ...
  update public.rule_templates set
    ...
    rules_meta=coalesce(rules_meta,'{}'::jsonb)||jsonb_build_object(
      'warlock_base_runtime',true,'runtime_revision','xphb-2024-warlock-base-runtime-v1',
      'mechanics_status','BASE_RUNTIME_READY','spell_progression','pact_magic','spellcasting_ability','charisma',
      'pact_magic_resource','warlock_pact_slots','mystic_arcanum_runtime',true,
      'invocation_runtime_included',false,'subclass_runtime_included',false,'source_book','XPHB','rules_revision','2024'
    ),updated_at=now()
  where id=v_warlock;
  ...
  for v_level in 1..20 loop
    ...
    if v_level=1 then
      v_m:=v_m||jsonb_build_array(private.warlock_feature(
        'warlock-base-pact-magic-rules','warlock-base:pact-magic','pact_magic','Магия договора',
        'Харизма — характеристика заклинаний. Все ячейки Магии договора одного уровня; заклинание 1–5 уровня тратит одну ячейку и сотворяется текущим уровнем ячейки колдуна. Потраченные ячейки восстанавливаются после короткого или долгого отдыха.',
    ...
    if v_level in (11,13,15,17) then
      declare
        v_spell_level integer := case v_level when 11 then 6 when 13 then 7 when 15 then 8 else 9 end;
        v_key text := 'warlock_mystic_arcanum_'||v_spell_level;
      begin
        v_m:=v_m||jsonb_build_array(
    ...
    if v_level=20 then
      v_m:=v_m||jsonb_build_array(private.warlock_feature(
        'warlock-base-eldritch-master-rules','warlock-base:eldritch-master','eldritch_master','Древнейший мастер','Магическая хитрость теперь восстанавливает все потраченные ячейки Магии договора. Ритуал остаётся минутным и восстанавливается после долгого отдыха.',jsonb_build_object('upgrades','magical_cunning','restore','all_pact_slots')));
    end if;
...
begin
  ...
  select * into v_state from public.character_resource_states
  where character_id=p_character_id and state_key='warlock_pact_slots' for update;
