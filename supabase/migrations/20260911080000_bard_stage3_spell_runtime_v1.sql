...
begin
  ...
  v_mechanics:=v_mechanics||jsonb_build_array(jsonb_build_object(
    'id','bard-spellcasting-focus-l1',
    'type','grant',
    'sourceKey','spellcasting',
    'target','permission',
    'key','spellcasting_focus:musical_instrument',
...
-- Defined after the installer body for readability; PostgreSQL resolves the
-- function reference when the installer executes, after the migration has
-- created this helper in the same transaction.
...
-- Recreate installer now that the slot helper exists.
create or replace function private.ensure_bard_spell_runtime_stage3_v1(p_campaign_id uuid)
returns void
language plpgsql
security definer
