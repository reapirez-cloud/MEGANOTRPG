-- Generic rest-refresh bridge for structured Choice Runtime v2.
-- Choices that explicitly declare refresh=short_rest or short_or_long_rest may be
-- replaced only while the matching authoritative rest window is open.
...
begin
  ...
  if v_refresh not in ('short_rest', 'short_or_long_rest') then
  ...
  if v_refresh = 'short_rest' and not v_short_open then
    raise exception 'CHOICE_SHORT_REST_WINDOW_CLOSED';
  end if;
...
'Generic Choice Runtime v2 rest refresh: replacement is permitted only inside authoritative short-rest or short/long-rest windows declared by the choice.';
