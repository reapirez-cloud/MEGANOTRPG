-- Keep AI-world resets safe for container deletes and restore the canonical
-- 2024 Warlock spell list entry for Armor of Agathys.
...
begin
  ...
  -- Deferred DELETE validation can run after the holder itself has already
  -- disappeared during a campaign cascade. No surviving holder means there
  -- is no remaining capacity invariant to validate.
  if not exists(
    select 1
...
begin
  for r in
    ...
    where t.catalog_key='class:warlock'
      and t.kind='class'
      and t.is_active=true
  loop
    perform private.install_warlock_pact_magic_selection_v2(r.campaign_id);
  end loop;
end;
$rebuild$;
