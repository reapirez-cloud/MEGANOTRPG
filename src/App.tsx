...
function Workspace(){
  ...
  if(route.type==="character")return <div className="app-shell"><CharacterProfileV2 characterId={route.id} onBack={goBack}/></div>
  ...
  return <div className="app-shell"><TopBar title={title} unreadCount={notifications.unreadCount} onOpenNotifications={()=>setNotificationsOpen(true)}/><main className="app-content">
    ...
    {route.tab==="me"&&!canManage&&activeCharacter&&<CharacterProfileV2 key={`${activeCharacter.id}:${characterRefreshKey}`} characterId={activeCharacter.id} onBack={()=>navigate("#/feed")} embedded/>}
