...
const PASSIVE_SKILLS: Record<PassiveKey, SkillKey> = {
  perception: "perception", investigation: "investigation", insight: "insight",
}
...
function resolveGrants(
  contributions: CharacterContribution[], state: CharacterState, maxHp: number,
): ResolvedGrant[] {
  const active = contributions
    .filter((contribution): contribution is GrantContribution =>
      contribution.kind === "grant" && evaluateCondition(contribution.condition, { state, maxHp }),
    )
    .sort(compareContributionOrder)
  const groups = new Map<string, { target: GrantContribution["target"]; key: string; variantKey: string; payload?: unknown; sources: ResolvedGrant["sources"] }>()
  for (const contribution of active) {
    ...
    const current = groups.get(identity) ?? { target: contribution.target, key: contribution.key, variantKey, payload: undefined, sources: [] }
    ...
    if (current.payload === undefined && contribution.payload !== undefined) current.payload = contribution.payload
    current.sources.push({ contributionId: contribution.id, source: contribution.source })
    groups.set(identity, current)
  }
  return [...groups.values()].filter((grant) => grant.sources.length > 0).sort((left, right) =>
    left.target.localeCompare(right.target) || left.key.localeCompare(right.key) || left.variantKey.localeCompare(right.variantKey),
  )
}

function proficiencyRank(value: ProficiencyRank | undefined): ProficiencyRank { return value ?? 0 }
...
export function resolveCharacter(
  ...
  const maxHp = resolveNumber("combat.maxHp", base.baseMaxHp, contributions, state, base.baseMaxHp)
  const proficiencyBonus = resolveNumber("core.proficiencyBonus", proficiencyBonusForLevel(base.level), contributions, state, maxHp.value)

  const abilities = Object.fromEntries(ABILITY_KEYS.map((ability) => {
    const resolved = resolveNumber(`abilities.${ability}`, base.abilities[ability], contributions, state, maxHp.value)
  ...
  const skills = Object.fromEntries(SKILL_KEYS.map((skill) => {
    ...
    const result: ResolvedSkill = {
      key: skill, ability, proficiencyRank: rank,
      bonus: resolveNumber(`skills.${skill}.bonus`, abilities[ability].modifier + proficiencyBonus.value * rank, contributions, state, maxHp.value),
    }
    return [skill, result]
  })) as Record<SkillKey, ResolvedSkill>

  const savingThrows = Object.fromEntries(ABILITY_KEYS.map((ability) => {
    const rank = proficiencyRank(base.savingThrowProficiencies?.[ability])
    const result: ResolvedSavingThrow = {
      ability, proficiencyRank: rank,
      bonus: resolveNumber(`savingThrows.${ability}.bonus`, abilities[ability].modifier + proficiencyBonus.value * rank, contributions, state, maxHp.value),
    }
    return [ability, result]
  ...
  const acFormulaContributions = contributions.filter((contribution): contribution is FormulaContribution =>
  ...
  const ac = { ...acNumeric, formula: acSelection.formula, formulaSources: acSelection.sources }

  const speed = resolveNumber("combat.speed", base.baseSpeed, contributions, state, maxHp.value)
  const initiative = resolveNumber("combat.initiative", abilities.dexterity.modifier, contributions, state, maxHp.value)
  const passives = Object.fromEntries(PASSIVE_KEYS.map((passive) => {
    const skill = PASSIVE_SKILLS[passive]
    return [passive, resolveNumber(`passives.${passive}`, 10 + skills[skill].bonus.value, contributions, state, maxHp.value)]
  })) as Record<PassiveKey, ResolvedNumber>

  const spellcastingByAbility = Object.fromEntries(ABILITY_KEYS.map((ability) => {
  ...
  return {
    id: base.id, name: base.name, level: base.level, proficiencyBonus, abilities, skills, savingThrows,
    combat: { ac, maxHp, currentHp: state.currentHp, tempHp: state.tempHp, speed, initiative },
    passives,
