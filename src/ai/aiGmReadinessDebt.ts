...
export const AI_GM_READINESS_DEBT: AiGmReadinessDebtItem[] = [
  ...
  {
    id: "world-maintenance-45-all-messages",
    title: "Сверка и архивация каждые 45 сообщений чата",
    removeWhenReady: "УДАЛИТЬ ПРИ РЕЙДИ",
    requirement:
      "Главному GM всегда передаются последние 50 сообщений комнаты независимо от автора. После каждых 45 новых сообщений чата фиксированный worker подводит итог окна, сверяет его с канонической базой и переносит подтверждённые устойчивые изменения в мир/квесты/NPC/отношения/фракции/память.",
    readyWhen: [
      "Серверный счётчик считает все новые chat_messages комнаты: игрока, Рассказчика, NPC и gameplay/event cards; технические записи вне chat_messages счётчик не увеличивают.",
      "GM context содержит последние 50 chat_messages в хронологическом порядке с author/character/event metadata.",
      "После каждых 45 новых chat_messages запускается ровно один идемпотентный reconcile/archive job с watermark по последнему обработанному message id.",
      ...
      "После успешной сверки подтверждённые факты/сводка получают provenance на диапазон обработанных message ids, а следующий порог 45 считается от нового watermark.",
      "Архивация не удаляет chat history: она создаёт компактный слой памяти/мира, а последние 50 сообщений остаются непосредственным conversational context.",
