...
export const AI_GM_READINESS_DEBT: AiGmReadinessDebtItem[] = [
  ...
  {
    id: "world-maintenance-20-narrative-messages",
    title: "World maintenance каждые 20 повествовательных сообщений",
    removeWhenReady: "УДАЛИТЬ ПРИ РЕЙДИ",
    requirement:
      "Фиксированный worker после каждых 20 narrative messages пакетно обновляет вторичное состояние мира, квестов, NPC, отношений, фракций и память.",
    readyWhen: [
      "Счётчик/watermark хранится серверно и идемпотентно.",
      "Roll/event/system/worker messages не раздувают narrative counter.",
      "Maintenance получает bounded event window и выполняет изменения через существующие owner/tools.",
      "Критическое текущее состояние не ждёт 20 сообщений: перемещение, смерть, трата предмета, открытие пути и однозначный quest transition применяются сразу.",
    ],
  },
  {
    id: "world-maintenance-memory",
    title: "Память и консолидация мира через worker",
    removeWhenReady: "УДАЛИТЬ ПРИ РЕЙДИ",
    requirement:
      "Worker поддерживает краткую сводку сцены/периода и долговременные факты на основе campaign_events, не подменяя ими канонический current state.",
    readyWhen: [
      "Summary/facts имеют provenance на исходные campaign_events.",
