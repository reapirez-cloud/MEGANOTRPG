...
function skillRanks(value: CharacterSheet["skill_proficiencies"]): Partial<Record<SkillKey, ProficiencyRank>> {
  return Object.fromEntries(Object.entries(value || {}).map(([key, rank]) => [key, Math.max(0, Math.min(2, Number(rank))) as ProficiencyRank])) as Partial<Record<SkillKey, ProficiencyRank>>
}
function savingThrowRanks(value: string[]): Partial<Record<AbilityKey, ProficiencyRank>> {
  const result: Partial<Record<AbilityKey, ProficiencyRank>> = {}; for (const raw of value || []) { const ability = ABILITY_ALIASES[normalize(raw)]; if (ability) result[ability] = 1 } return result
}
...
function configuredSlotLevels(sheet: CharacterSheet, spells: CharacterSpell[]): number[] {
  const levels = new Set<number>(); for (let level = 1; level <= 9; level += 1) if (Number(sheet.spell_slots?.[String(level)]?.max || 0) > 0) levels.add(level)
  for (const spell of spells) if (spell.spell_level > 0 && spell.cast_mode !== "cantrip") levels.add(spell.spell_level)
...
export function buildLegacyCharacterEngineInput(args: {
  character: Pick<Character, "id" | "name" | "level">; sheet: CharacterSheet; spells: CharacterSpell[]; features: CharacterFeature[]; inventory?: InventoryItem[]
  ...
  if (sheet.spellcasting_enabled) for (const spell of spells) {
    ...
    contributions.push({ id: `legacy:spell:${spell.id}`, kind: "grant", operation: "GRANT", target: "spell", key: legacySpellKey(spell), variantKey: `legacy-${spell.id}`, payload: {
      spell: { name: spell.name, level: spell.spell_level, ...(spell.school.trim() ? { school: spell.school.trim() } : {}), ritual: spell.ritual },
      preparation: { mode: "not_required" }, methods: [{ key: "legacy-cast", kind: "spellcasting", ...(spellcastingAbility ? { ability: spellcastingAbility } : {}), requiresPrepared: false, ...(isCantrip ? {} : { resourceOptions: options }) }],
  ...
  return { base: {
    ...
    abilities: { strength: sheet.strength, dexterity: sheet.dexterity, constitution: sheet.constitution, intelligence: sheet.intelligence, wisdom: sheet.wisdom, charisma: sheet.charisma },
    baseMaxHp: sheet.max_hp, baseSpeed: sheet.speed, skillProficiencies: skillRanks(sheet.skill_proficiencies), savingThrowProficiencies: savingThrowRanks(sheet.saving_throw_proficiencies),
  }, state: { currentHp: sheet.current_hp, tempHp: sheet.temp_hp, resources }, contributions }
}

export function resolveLegacyCharacterEngineView(args: { character: Pick<Character, "id" | "name" | "level">; sheet: CharacterSheet; spells: CharacterSpell[]; features: CharacterFeature[]; inventory?: InventoryItem[] }): LegacyCharacterEngineView {
