...
export default function CharacterGameFrame({ characterId, children }: Props) {
  ...
  return <div className={`character-game-frame ${lifeState === "dead" ? "is-dead" : ""}`}>
    ...
    {sheetOpen && <div className="soft-sheet-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) setSheetOpen(false) }}><section className="soft-sheet character-game-admin-sheet"><div className="soft-sheet__handle"/><header className="soft-sheet__header"><div><small>Персонаж</small><h2>{character?.name || "Игровые правила"}</h2></div><button type="button" className="soft-sheet__close" onClick={() => setSheetOpen(false)}>×</button></header>
      <section className="character-admin-section"><div className="character-admin-section__head"><div><small>Статус</small><h3>Состояние персонажа</h3></div><span className={`life-pill life-pill--${lifeState}`}>{lifeState === "dead" ? "Мёртв" : "Жив"}</span></div><div className="life-actions"><button type="button" className={lifeState === "alive" ? "is-active" : ""} disabled={saving} onClick={() => void changeLife("alive")}>Жив</button><button type="button" className={lifeState === "dead" ? "is-danger is-active" : "is-danger"} disabled={saving} onClick={() => void changeLife("dead")}>† Мёртв</button></div><p>Смерть автоматически закрывает персональный чат для действий и снимает персонажа с активного статуса.</p></section>

      <section className="character-admin-section"><div className="character-admin-section__head"><div><small>Ресурсы</small><h3>Отдых и восстановление</h3></div><span className="life-pill">{runtime.rows.length} ресурсов</span></div><div className="resource-recovery-grid"><button type="button" disabled={saving} onClick={() => void recover("short_rest")}>◷ Короткий отдых</button><button type="button" disabled={saving} onClick={() => void recover("long_rest")}>☾ Долгий отдых</button><button type="button" disabled={saving} onClick={() => void recover("dawn")}>☀ Рассвет</button><button type="button" disabled={saving} onClick={() => void recover("manual")}>↻ Восстановить ручные</button></div><p>Каждый запас восстанавливается по правилам своей способности. Долгий отдых также возвращает HP и ячейки заклинаний.</p></section>

      <section className="character-admin-section"><div className="character-admin-section__head"><div><small>Правила</small><h3>Назначенные шаблоны</h3></div></div><div className="assigned-template-list">{assignedItems.map((bundle) => <div className="assigned-template" key={bundle.assignment.id}><span className="assigned-template__icon">{bundle.template.kind.includes("race") ? "◈" : "◇"}</span><span><small>{kindLabel[bundle.template.kind]}{bundle.assignment.template_level ? ` · ${bundle.assignment.template_level} ур.` : ""}</small><strong>{bundle.template.name}</strong></span><button type="button" disabled={saving} onClick={() => void removeAssignment(bundle.assignment.id)}>×</button></div>)}{!assignedItems.length && <div className="template-assignment-empty">Шаблоны ещё не назначены.</div>}</div></section>

      <section className="character-admin-section"><div className="character-admin-section__head"><div><small>Назначение</small><h3>Раса, класс и специализация</h3></div></div><div className="template-kind-switch template-kind-switch--four">{assignmentKinds.map((entry) => <button type="button" key={entry} className={kind === entry ? "is-active" : ""} onClick={() => chooseKind(entry)}>{kindLabel[entry]}</button>)}</div><select className="app-select" value={templateId} onChange={(event) => chooseTemplate(event.target.value)}><option value="">Выберите шаблон</option>{templatesForKind.map((template) => <option key={template.id} value={template.id}>{template.name}</option>)}</select>{kind === "class" && chosenTemplate && <label className="template-level-field"><span>Уровень в этом классе</span><input className="app-input" type="number" min="1" max="30" value={templateLevel} onChange={(event) => setTemplateLevel(Math.max(1, Math.min(30, Number(event.target.value) || 1)))}/></label>}
