...
export default function ChatActionSheet({ characterName, contract, loading = false, onClose, onFreeRoll, onCheck, onAction, onSpell }: Props) {
  ...
  return <div className="chat-action-backdrop" onMouseDown={onClose}><section className="chat-action-flow chat-action-flow--v3" onMouseDown={(event) => event.stopPropagation()}>
    ...
    <nav className="action-v3-tabs" aria-label="Тип действия">{tabs.map((item) => <button key={item.key} className={tab === item.key ? "is-active" : ""} type="button" onClick={() => setTab(item.key)}><span>{item.label}</span>{Boolean(item.count) && <small>{item.count}</small>}</button>)}</nav>
    <div className="action-v2-body">
      ...
      {!loading && contract && tab === "spells" && <>{contract.spells.length ? <div className="action-v2-list action-v2-list--cards">{contract.spells.map((spell) => <button disabled={busy || !spell.available} type="button" key={spell.key} onClick={() => void run(() => onSpell(spell))}><i>✧</i><span><strong>{spell.identity.name}</strong><small>{spellSummary(spell)}</small></span><em>›</em></button>)}</div> : <div className="action-v2-empty"><span>✧</span><strong>Нет доступной магии</strong><p>Заклинания собираются Character Engine из всех источников.</p></div>}</>}
