...
function SourceGroup({ group, kind, resources, labels, busy, onAction, onSpell }: {
  ...
  const choice = templateChoiceEffect(action)
  if (!choice) return [<button disabled={busy || !action.available} type="button" key={`${action.key}:${action.variantKey}`} onClick={() => onAction(action)}><i>{action.attack ? "⚔" : "◆"}</i><span><strong>{action.label || action.key}</strong><small>{actionSummary(action, resources, labels)}</small></span><em>›</em></button>]
  return choice.options.map((option) => <button disabled={busy || !action.available} type="button" key={`${action.key}:${action.variantKey}:${option}`} onClick={() => onAction(action, option)}><i>◆</i><span><strong>{choice.optionLabels?.[option] || option}</strong><small>{action.label || action.key} · {actionSummary(action, resources, labels)}</small></span><em>›</em></button>)
