...
export default function ChatPreparationCard({ roomId, characterId, model, onOpenCharacter, onChanged }: Props) {
  if (!model.session?.is_open || model.tasks.length === 0) return null
  const spellTasks = model.tasks.filter((task) => task.kind === "spells")
  ...
  return <aside className="rest-prep-card">
    ...
    {spellTasks.map((task) => <section className="rest-prep-task" key={task.key}>
      <div className="rest-prep-task__head"><span>✧</span><div><small>{task.sourceName}</small><strong>Подготовить заклинания</strong></div></div>
      <button className="rest-prep-open-sheet" type="button" onClick={onOpenCharacter}>Открыть заклинания персонажа</button>
    </section>)}
