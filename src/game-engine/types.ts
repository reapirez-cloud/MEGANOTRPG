...
export type GenaCommand =
  ...
  | { kind: "inventory.use"; context: EngineCommandContext; characterId: string; itemId: string; amount?: number; expectedVersion?: number; label: string }
  | { kind: "inventory.transfer"; context: EngineCommandContext; fromCharacterId: string; toCharacterId: string; itemId: string; amount: number; expectedVersion?: number }
