...
export default function CharacterProfileV2({ characterId, onBack, embedded = false }: Props) {
  ...
  return (
    <div data-class-key={classId || "default"} className={`screen character-profile-screen character-profile-v2 character-profile-opus ${embedded ? "character-profile-screen--embedded" : ""}`}>
      ...
      {editor?.type === "inventory" && canManage && <InventoryItemEditor item={editor.item} campaignId={campaignId} enablePhysicalProfile onClose={() => setEditor(null)} onSave={(input, inventoryProfile) => editor.item ? data.updateInventoryItem(editor.item.id, input, inventoryProfile) : data.addInventoryItem(input, inventoryProfile)} onDelete={editor.item ? () => data.deleteInventoryItem(editor.item!.id) : undefined} />}
