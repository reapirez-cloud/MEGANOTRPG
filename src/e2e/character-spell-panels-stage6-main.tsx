...
function spell(
  ...
  return {
    ...
    name,
    spell_level: level,
    school: level === 0 ? "Evocation" : concentration ? "Enchantment" : "Abjuration",
    ...
    description: `${name}: тестовое описание для финальной визуальной сертификации панели заклинаний.`,
    source: "Жрец",
    sort_order: level,
    created_at: now,
    updated_at: now,
...
const spells: CharacterSpell[] = [
  ...
  spell("silence", "Молчание", 2, "Действие", true, true),
  spell("lesser-restoration", "Малое восстановление", 2, "Действие"),
  spell("detect-thoughts", "Обнаружение мыслей", 2, "Действие", true),
  spell("spiritual-weapon", "Духовное оружие", 2, "Бонусное действие"),

  spell("remove-curse", "Снятие проклятия", 3, "Действие"),
  spell("circle-of-protection", "Круг защиты от всего неприятного и очень длинного", 3, "Действие", true),
  spell("revivify", "Оживить мёртвых", 3, "Действие"),

  spell("banishment", "Изгнание", 4, "Действие", true),
  spell("death-ward", "Защита от смерти", 4, "Действие"),

  spell("greater-restoration", "Высшее восстановление", 5, "Действие"),
]
...
function Stage6SpellPanelPreview() {
  const contract = useMemo(() => {
    const input = buildLegacyCharacterEngineInput({
      character: { id: characterId, name: "Аурелия", level: 5 },
      sheet,
      spells,
      features: [],
    })
...
const root = document.getElementById("character-spell-panels-stage6-root")
if (!root) throw new Error("Stage 6 spell panel root not found")
