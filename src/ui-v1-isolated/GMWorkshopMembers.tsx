...
import {
  ...
  createWorkshopMemberSetActiveAction,
  createWorkshopMemberUnassignAction,
...
export default function GMWorkshopMembers({
  data,
  onOpenCharacter,
}: {
  ...
  useAIViewContextLayer(
    "gm-workshop-members",
    {
      screen: "gm-workshop-members",
      title: member ? "Участники · " + member.displayName : "Мастерская · Участники",
      text: member
        ? "GM открыл конкретного участника кампании."
        : "GM управляет участниками, ролями, приглашениями и назначением PC.",
      entity: member
        ? {
  ...
  function openInviteAction(actionId: string) {
    const action = createWorkshopInviteActions({
      invite: data.invite,
          ...
          <div className="u1-gm-list">
            ...
            {!assigned.length && (
              <div className="u1-gm-empty">
                Этому участнику ещё не назначен PC.
              </div>
            )}
          </div>
        </section>
          ...
          <small>
            {data.invite
              ? "Осталось " +
                ...
                " из " +
                data.invite.maxUses
              : "Создай код и отправь будущему участнику."}
          </small>
          <div>
            {data.invite && (
              <button type="button" onClick={() => openInviteAction("copy-invite")}>
                Копировать
              </button>
            )}
            <button type="button" onClick={() => openInviteAction("create-invite")}>
