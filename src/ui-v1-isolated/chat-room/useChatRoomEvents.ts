  ...
  const loadInitial = useCallback(async () => {
    ...
    const [messagesResult, gmUsers] = await Promise.all([
      supabase
        .from("chat_messages")
        .select(
          "id, room_id, user_id, client_id, character_id, author_name, author_avatar_url, body, created_at, edited_at, attachment_url, attachment_kind, event_kind, event_payload, turn_command_id, turn_component, turn_order, audience_scope, recipient_character_ids",
        )
        .eq("room_id", roomId)
