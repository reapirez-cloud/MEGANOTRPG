...
import {
  compileWorkshopItemInput,
  workshopFeatureInput,
  workshopLinkedDefinitionRefs,
  workshopSpellInput,
} from "./gmWorkshopDefinitionRuntime"
  ...
  const operations = useMemo<WorkshopOperations>(() => ({
    ...
    issueDefinition(definition, characterId, quantity) {
      if (definition.kind === "item") {
        return mutate(
          async () => oracle.inventory.create(
            context(),
            characterId,
            await compileWorkshopItemInput(definition, quantity),
          ),
      ...
      if (definition.kind === "spell") {
        return mutate(
          () => oracle.characters.createSpell(context(), characterId, workshopSpellInput(definition)),
          "Не удалось выдать заклинание.",
        )
      }

      if (definition.kind === "feature" || definition.kind === "condition" || definition.kind === "feat") {
        const input = workshopFeatureInput(definition)
        if (!input) {
          return Promise.resolve({ ok: false, error: "Не удалось собрать runtime-способность." })
        }
        return mutate(
          () => oracle.characters.createFeature(context(), characterId, input),
    ...
    async linkDefinitionToItem(definition, itemDefinitionId) {
      ...
      const linked = workshopLinkedDefinitionRefs(target)
